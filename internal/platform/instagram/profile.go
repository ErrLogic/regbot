...
// UpdateProfile updates the Instagram profile bio, display name, or avatar.
func UpdateProfile(
	ctx context.Context,
	...
	logFunc("info", "profile", "Starting profile update flow")
	...
	// Add optional steps based on params.
	if params.DisplayName != "" {
		steps = append(steps, struct {
			...
			func(ctx context.Context) error {
				return flows.TypeByLocator(ctx, driver, loc, "display_name_field", params.DisplayName, wait)
			},
		})
	}
	if params.Bio != "" {
	...
	steps = append(steps, struct {
		...
		"save profile",
		func(ctx context.Context) error {
			return flows.TapByLocator(ctx, driver, loc, "save_profile_button", wait)
		},
	})
	...
	logFunc("info", "profile", "Profile updated successfully")
	return nil
}
